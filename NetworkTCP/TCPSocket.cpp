...
BOOL CTCPSocket::CreateServer(int nPort,int backlog)
{
	...
	local.sin_addr.s_addr=inet_addr(sIpAddress);
...
DWORD WINAPI CTCPSocket::ServerThread(LPVOID lpParameter)
{
	...
	while(m_pTCP->m_bAuto)
	{
		...
		char charIP[16];
		BOOL isIP = FALSE;
		sprintf(charIP,"%3d.%3d.%3d.%3d",addr.sin_addr.S_un.S_un_b.s_b1,addr.sin_addr.S_un.S_un_b.s_b2,addr.sin_addr.S_un.S_un_b.s_b3,addr.sin_addr.S_un.S_un_b.s_b4);
		for(i=0;i<MAX_CONNECTION;i++)
		{
			if ( stricmp(charIP,m_pTCP->m_cIp[i]) == 0)
			{
				isIP = true;
				break;
			}
...
DWORD WINAPI CTCPSocket::DataThread(LPVOID lpParameter)
{
	...
	int nRet;
	char buf[512];
	static int currlen=0, lastbuff[CHUNK_LEN*2];
	...
	while(m_pTCP->m_bConnected[MyNumber])
	{
		...
		{
			nRet=recv(m_pTCP->m_sServer[MyNumber],buf,CHUNK_LEN,0);
...
//#endif
	/*			currlen+=nRet;
				if(currlen >= CHUNK_LEN)
				{
					while(currlen >= CHUNK_LEN)
					{
						if(m_pTCP->m_lpServerDataArriveProc!=NULL)
						{
							char* inf;
							inf=new char[currlen+21];
							sprintf(inf,"S%s%3d",m_pTCP->m_cIp[MyNumber],MyNumber);
							//memcpy(inf+21,buf,nRet);
							if(currlen > nRet)
							{
								memcpy(inf+21, lastbuff, currlen-nRet);
								memcpy(inf+21+currlen-nRet, buf, CHUNK_LEN+nRet-currlen);
								memcpy(lastbuff, buf+CHUNK_LEN+nRet-currlen, currlen-CHUNK_LEN);
							}
							else
							{
								memcpy(inf+21, buf, CHUNK_LEN);
							}
							//m_pTCP->m_lpServerDataArriveProc(inf,nRet+21,m_pTCP->m_dwUserData);
							m_pTCP->m_lpServerDataArriveProc(inf,CHUNK_LEN+21,m_pTCP->m_dwUserData);
							delete inf;
						}
						currlen-=CHUNK_LEN;
					}
				...
				{
					memcpy(lastbuff+currlen-nRet, buf, nRet);
				}
				continue;*/
			}
